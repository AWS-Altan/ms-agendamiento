...
$manda ='curl -X POST "'.$url.'" -H "authorization: Bearer '.$token.'" -H "cache-control:no-cache" -H "content-type: application/json" -d "{\"eventType\":\"TEST\",\"callback\":\"'.$call.'\",\"event\":{\"id\":\"'.$id_com.'\",\"effectiveDate\":\"'.$timestamp.'\",\"detail\":\"Order cancelled\"}}" 2> /dev/null';
