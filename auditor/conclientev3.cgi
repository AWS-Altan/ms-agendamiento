...
$io='curl -X GET "'.$url.''.$msisdn.'/profile"   -H "OperatorId: '.$user.'"  -H  "accept: application/json" -H  "Authorization: Bearer '.$token.' "  -H  "Operation-User: '.$user2.' " -H  "Operation-Password: '.$pass.'   " -H  "PartnerId: '.$id_com.' " 2> /dev/null';
&comova($io);
use Data::Dumper qw(Dumper);
use JSON qw(decode_json);
my $json  = JSON->new->utf8;
