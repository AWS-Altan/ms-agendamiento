...
if(length($im)!=15 && $resor=~'Inactivations' && $resor=~'Activations')
{
$f=1;

}
if(length($dn)!=10 && $resor!~'Inactivations' && $resor!~'Activations' )
{
$f=1;
}
...
if($resor=~'Inactivations' && $resor=~'Activations')
{
$strsql="SELECT    count(*) FROM auditor.auditor_on_line_batch  WHERE id_company='$be' and id_estatus=1 and imsi='$im'   ";
}
else{
...
if($ddd==0)
{
$strsql="insert into auditor.auditor_on_line_batch (id_company,id_estatus,msisdn,oferta,address,imsi,fecha_ingreso,transaccion_id,operacion) values('$be','1','$dn','$ofer','$coe',$im,'$fe','$id','$resor')";
