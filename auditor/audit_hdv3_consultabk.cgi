...
$strsql="SELECT  *   FROM auditor.auditor_on_line_batch  WHERE id_company='$be' and msisdn='$msisdn' and fecha_ingreso > date_sub(now(), interval 3 month)  ";
#print $strsql;
...
$as1x.='{"id":"'.$id.'","status":"'.$ddde.'","offeringId":"'.$ofer.'","coordinates":"'.$dir.'","effectivedate":"'.$fe.'","operation":"'.$op.'"},';
...
$as1= $json->encode({be_id=>$be,status=>'no-ok',description=>'No found data',transationDate=>$fecg});


}
}
else{
$as1= $json->encode({be_id=>$be,status=>'no-ok',description=>'some the data is not correct',id_transation_cudar=>$id,transationDate=>$fecg});
}
