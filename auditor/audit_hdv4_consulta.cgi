...
sub escribelog{
  ...
  close (LOG);
  my $log_file2="/var/www/cgi-bin/log/eventos/resultado-agendaonline-$ao-$mes-$dia-$id_com.log";
  open (LOG," >> $log_file2");
  print LOG "$ao-$mes-$dia-$hora:$min:$seg-$error\n";
  close (LOG);
}
...
$as1x.='{"id":"'.$id.'","status":"'.$ddde.'","offeringId":'.$OFFID.',"coordinates":"'.$dir.'","effectivedate":"'.$fe.'","operation":"'.$op.'","purchasedSequency":"'.$sec.'","expireEffectiveDate":"'.$fexp.'","startEffectiveDate":"'.$fi.'","order_bss":"'.$oid_bss.'"},';
&escribelog($as1x);
